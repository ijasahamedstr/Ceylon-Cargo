...
import UpdateBookingCustomer from "../Bookings/UpdateBookingCustomer"; // Assumed path based on View

const API_BASE_URL = import.meta.env.VITE_API_URL || "http://localhost:8001";
const CACHE_KEY = "BOOKING_CUSTOMER_VAULT_SHIPMENT_MANIFEST";
const PRIMARY_TEAL = "#004652";
const ACCENT_GOLD = "#CC9D2F";

// 🔒 This page is locked to a single booking stage
const PAGE_STATUS = "shipment_manifest";
          ...
          <Box px={isMobile ? 1 : 0}>
            {!isMobile && (
              <Breadcrumbs separator={<NavigateNext sx={{ fontSize: "0.8rem" }} />} sx={{ mb: 0.5 }}>
                ...
                <Typography sx={{ fontSize: "0.65rem", fontWeight: 800, color: PRIMARY_TEAL, letterSpacing: 1 }}>SHIPMENT MANIFEST</Typography>
              </Breadcrumbs>
            )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                ...
                {paginatedData.map(item => (
                  <motion.div key={item._id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                    <Card elevation={0} sx={{ border: "1px solid", borderColor: selected.includes(item._id) ? PRIMARY_TEAL : "#E2E8F0", bgcolor: selected.includes(item._id) ? "#F4F8F9" : "white", transition: "all 0.2s ease" }}>
                      ...
                      {/* 🛠 Mobile Action Bar (View, Edit, Delete) 🛠 */}
          ...
          {paginatedData.length === 0 && (
            <Paper elevation={0} sx={{ p: isMobile ? 4 : 8, textAlign: "center", borderRadius: "12px", border: "2px dashed #E2E8F0", mt: 2, bgcolor: "transparent" }}>
              ...
              <Typography sx={{ color: "#64748B", fontWeight: 700, fontSize: "1rem" }}>No bookings currently in Shipment Manifest stage.</Typography>
