...
const ManageShipment = ({ shipmentData, onBack }: Props) => {
  ...
  const fetchItems = useCallback(async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/api/shipments/${shipmentData._id}`);
  ...
  const handleAddItem = async () => {
    setSaving(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/shipments/${shipmentData._id}/items`, {
        method: "POST",
  ...
  const handleDeleteItem = async () => {
    ...
    setDeleteId(null);
    try {
      await fetch(`${API_BASE_URL}/api/items/${deleteId}`, { method: "DELETE" });
