...
const ShipmentManager = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`${API_BASE_URL}/api/shipments/${id}`, { method: "DELETE", headers: authHeaders() });
  ...
  const handleBulkDelete = async () => {
    ...
    ids.forEach(async id => {
      try { await fetch(`${API_BASE_URL}/api/shipments/${id}`, { method: "DELETE", headers: authHeaders() }); } catch {}
    });
  };
