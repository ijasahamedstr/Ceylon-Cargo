...
import {
  ...
  FlightTakeoffOutlined, DirectionsBoatOutlined, QrCode2Outlined,
  ArrowForwardIosOutlined, EditOutlined, DeleteOutline,
  WarningAmberRounded, HistoryToggleOffOutlined, CheckCircleOutline,
  LocalShippingOutlined
} from "@mui/icons-material";

// Import your views (Ensure these paths are correct for your project)
...
const WarehouseSL = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    notify(`Deleted ${count} bookings.`, "success");
  ...
  const handleBulkStatusUpdate = async (event: any) => {
    ...
    // Optimistic UI Update - Removes from current view if status changes
    ...
    notify(`Moved ${count} bookings to ${formattedStatusName}.`, "success");
    setHistory(prev => [`Moved ${count} items to ${formattedStatusName}`, ...prev].slice(0, 8));
  ...
  const getStatusChip = (status: string) => {
    ...
    switch (s) {
      case "collect_item": 
      case "collect item": return <Chip label="Collect Item" size="small" sx={{ bgcolor: "#FFFBEB", color: "#D97706", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      case "move_to_warehouse_sa":
      case "move to warehouse sa": return <Chip label="Warehouse SA" size="small" sx={{ bgcolor: "#EFF6FF", color: "#2563EB", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      case "loading_box":
      case "loading box": return <Chip label="Loading Box" size="small" sx={{ bgcolor: "#F1F5F9", color: "#475569", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      case "shipment_manifest":
      case "shipment manifest": return <Chip label="Manifest" size="small" sx={{ bgcolor: "#F5F3FF", color: "#7C3AED", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      case "arrived_warehouse_sl":
      case "arrived warehouse sl": return <Chip label="Warehouse SL" size="small" sx={{ bgcolor: "#ECFDF5", color: "#059669", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      case "delivered": return <Chip label="Delivered" size="small" sx={{ bgcolor: "#F0FDF4", color: "#16A34A", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      case "cancelled": return <Chip label="Cancelled" size="small" sx={{ bgcolor: "#FEF2F2", color: "#DC2626", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
      default: return <Chip label={status ? status.replace(/_/g, " ").toUpperCase() : "DRAFT"} size="small" sx={{ bgcolor: "#F8FAFC", color: "#64748B", fontWeight: 700, fontSize: "0.65rem", borderRadius: "4px" }} />;
    }
  };
  ...
  return (
    <ThemeProvider theme={montserratTheme}>
      <Box sx={{ width: "100%", minHeight: "100vh", bgcolor: "#F4F7FA", p: { xs: 1, sm: 2, md: 3 }, pb: { xs: 10, md: 3 } }}>
          ...
          <Box px={isMobile ? 1 : 0}>
            {!isMobile && (
              <Breadcrumbs separator={<NavigateNext sx={{ fontSize: "0.8rem" }} />} sx={{ mb: 0.5 }}>
                ...
                <Typography sx={{ fontSize: "0.65rem", fontWeight: 800, color: PRIMARY_TEAL, letterSpacing: 1 }}>WAREHOUSE SL</Typography>
              </Breadcrumbs>
            )}
            <Typography variant="h5" sx={{ fontWeight: 800, color: PRIMARY_TEAL, fontSize: isMobile ? "1.4rem" : "1.8rem", letterSpacing: "-0.5px" }}>
              Arrived at Warehouse SL
            </Typography>
        ...
        <Paper elevation={0} sx={{ p: isMobile ? 2 : 3, mb: 3, borderRadius: isMobile ? "16px" : "12px", border: "1px solid #E2E8F0", bgcolor: "white", boxShadow: "0px 2px 4px rgba(0,0,0,0.02)" }}>
          <Stack spacing={isMobile ? 1.5 : 2}>
            ...
            <Stack direction={{ xs: "column", sm: "row" }} spacing={isMobile ? 1.5 : 2} alignItems="stretch" flexWrap="wrap">
              <Stack direction="row" spacing={isMobile ? 1.5 : 2} flexGrow={1}>
                <FormControl size="small" sx={{ flexGrow: 1 }}>
                  ...
                  <Select
                    ...
                    label="Cargo Type" sx={{ borderRadius: "8px", fontSize: "0.85rem", fontWeight: 600, bgcolor: "white" }}
                  >
                ...
                <FormControl size="small" sx={{ flexGrow: 1 }}>
                  ...
                  <Select
                    ...
                    label="Payment Status" sx={{ borderRadius: "8px", fontSize: "0.85rem", fontWeight: 600, bgcolor: "white" }}
                  >
              ...
              <Stack direction="row" spacing={1.5} flexGrow={1} alignItems="center">
                <TextField
                  type="date" size="small" value={filterDate} onChange={e => setFilterDate(e.target.value)}
                  sx={{ flexGrow: 1, "& .MuiInputBase-root": { borderRadius: "8px", fontSize: "0.85rem", fontWeight: 600, bgcolor: "white" } }}
                />

                {(filterCargoType || filterPaymentStatus || filterDate || searchQuery) && (
                  <Button 
                    onClick={handleClearFilters} variant="text" color="error"
                    sx={{ minWidth: "auto", px: 2, fontWeight: 700, fontSize: "0.8rem", borderRadius: "8px" }}
                  >
                    Clear
                  </Button>
                )}
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
                ...
                {paginatedData.map(item => (
                  <motion.div key={item._id} initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                    <Card elevation={0} sx={{ border: "1px solid", borderColor: selected.includes(item._id) ? PRIMARY_TEAL : "#E2E8F0", bgcolor: selected.includes(item._id) ? "#F4F8F9" : "white", transition: "all 0.2s ease" }}>
                      ...
                      <Divider sx={{ mx: 2, mb: 1.5, borderColor: "#F1F5F9" }} />
                      
                      <Stack direction="row" alignItems="center" sx={{ px: 2, mb: 1.5 }}>
                      ...
                      {/* 🛠 Interactive Action Bar (View, Edit, Delete) 🛠 */}
                      ...
                      >
                        ...
                        <TableCell>
                          <Stack direction="row" spacing={1} alignItems="center">
                            ...
                            <Typography sx={{ fontSize: "0.8rem", fontWeight: 800, color: "#475569", textTransform: "uppercase" }}>
                              {item.cargo_type || "-"}
                            </Typography>
                          </Stack>
                        </TableCell>
                        ...
                        {/* 🛠 Desktop Action Column (View, Edit, Delete) 🛠 */}
                        <TableCell>
                          <Stack direction="row" spacing={1}>
                            <Tooltip title="View Details">
                              <IconButton size="small" onClick={() => { setSelectedItem(item); setView("view"); }}
                                sx={{ color: "#64748B", border: "1px solid #E2E8F0", borderRadius: "8px", "&:hover": { bgcolor: "white", color: PRIMARY_TEAL, borderColor: PRIMARY_TEAL } }}>
                            ...
                            <Tooltip title="Edit Booking">
                              <IconButton size="small" onClick={() => { setSelectedItem(item); setView("edit"); }}
                                sx={{ color: "#64748B", border: "1px solid #E2E8F0", borderRadius: "8px", "&:hover": { bgcolor: "white", color: "#1E293B", borderColor: "#1E293B" } }}>
                                <EditOutlined fontSize="small" />
                              </IconButton>
                            </Tooltip>
                            ...
                            <Tooltip title="Delete">
                              <IconButton size="small" onClick={() => setDeleteDialog({ open: true, id: item._id })}
                                sx={{ color: "#EF4444", border: "1px solid transparent", borderRadius: "8px", "&:hover": { bgcolor: "#FEF2F2", borderColor: "#FECACA" } }}>
                                <DeleteOutline fontSize="small" />
                              </IconButton>
                            </Tooltip>
                          </Stack>
          ...
          {paginatedData.length === 0 && (
            <Paper elevation={0} sx={{ p: isMobile ? 4 : 8, textAlign: "center", borderRadius: "12px", border: "2px dashed #E2E8F0", mt: 2, bgcolor: "transparent" }}>
              ...
              <Typography sx={{ color: "#64748B", fontWeight: 700, fontSize: "1rem" }}>No bookings currently in Warehouse SL.</Typography>
              <Typography sx={{ color: "#94A3B8", fontWeight: 500, fontSize: "0.85rem", mt: 1 }}>Adjust your search filters or check other stages.</Typography>
            </Paper>
          )}
        </motion.div>
