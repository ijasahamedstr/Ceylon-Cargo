...
const AdminManagement: React.FC = () => {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await apiClient.delete(`/api/delete/${targetId}`);
...
const AdminForm = ({ admin, isEditing, onSave, loading }: any) => {
  const [formData, setFormData] = useState({
    ...
    allowedModules: admin?.allowedModules || [...ALL_MODULES]
  });

  const handleCheckboxChange = (module: string) => {
    setFormData(prev => {
      const allowed = prev.allowedModules.includes(module)
        ? prev.allowedModules.filter((m: string) => m !== module)
    ...
    <Paper elevation={0} sx={{ p: 5, borderRadius: "24px", border: "1px solid #E2E8F0", maxWidth: "650px", mx: "auto", bgcolor: '#FFF' }}>
      <Stack spacing={4}>
        ...
        <Stack spacing={2.5}>
          ...
          <Box>
            <Typography variant="subtitle2" sx={{ fontWeight: 800, color: PRIMARY_TEAL, mb: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              Module Access Control
            </Typography>
            <Grid container spacing={1}>
              {ALL_MODULES.map((module) => (
                <Grid size={{ xs: 12, sm: 6 }} key={module}>
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        sx={{
                          color: PRIMARY_TEAL,
                          '&.Mui-checked': { color: PRIMARY_TEAL },
                        }}
                      />
                    }
                    label={
                      <Typography sx={{ fontFamily: PRIMARY_FONT, fontWeight: 700, fontSize: '0.82rem', color: '#475569' }}>
                        {module}
                      </Typography>
                    }
                  />
...
const AdminDetails = ({ admin: initialAdmin, onEdit, onDelete }: any) => {
  ...
  const handleSetup2FA = async () => {
    setLoading2FA(true);
    try {
      const res = await apiClient.post("/api/setup-2fa", { adminId: admin._id });
  ...
  const handleVerifyAndEnable = async () => {
    setLoading2FA(true);
    try {
      const res = await apiClient.post("/api/verify-2fa", { adminId: admin._id, token: verificationCode });
      if (res.data.success) {
  ...
  return (
    <Paper elevation={0} sx={{ p: 4, borderRadius: "24px", border: "1px solid #E2E8F0", maxWidth: '800px', mx: 'auto', bgcolor: '#FFF' }}>
      <Stack spacing={4}>
        ...
        <Paper variant="outlined" sx={{ p: 3, borderRadius: '15px', border: '1px solid #E2E8F0' }}>
          <Typography variant="subtitle2" sx={{ fontWeight: 800, color: PRIMARY_TEAL, mb: 2, textTransform: 'uppercase', letterSpacing: 0.5 }}>
            Module Access Permissions
          </Typography>
          <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
            {(admin.allowedModules && admin.allowedModules.length > 0) ? (
              admin.allowedModules.map((m: string) => (
                <Chip key={m} label={m} size="small" sx={{ fontWeight: 700, bgcolor: '#F0F5F6', color: PRIMARY_TEAL, fontFamily: PRIMARY_FONT }} />
              ))
            ) : (
              <Typography sx={{ fontSize: '0.8rem', color: '#94A3B8', fontWeight: 500, fontStyle: 'italic' }}>
                No modules allowed (Access Denied to all dashboard options).
              </Typography>
            )}
          </Box>
        </Paper>
