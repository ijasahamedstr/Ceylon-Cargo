...
export default function PortManager() {
  ...
  const fetchPorts = useCallback(async () => {
    try {
      ...
      notify("Failed to fetch ports", "error");
    }
  }, []);
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const data = await portService.create(newCode.toUpperCase(), newName);
      if (data.success) {
        setNewCode("");
        setNewName("");
        fetchPorts();
        notify("Port added", "success");
      }
    } catch (err: any) {
      const msg = err.response?.data?.message || "Failed to add port";
      notify(msg, "error");
    }
    setSaving(false);
  };
  ...
  const handleUpdate = async () => {
    ...
    setSaving(true);
    try {
      const data = await portService.update(editRow._id, editRow.code, editRow.name);
      if (data.success) {
        setEditRow(null);
        fetchPorts();
        notify("Port updated", "success");
      }
    } catch (err: any) {
      const msg = err.response?.data?.message || "Failed to update port";
      notify(msg, "error");
    }
    setSaving(false);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await portService.delete(deleteId);
      fetchPorts();
      notify("Port deleted", "success");
    } catch {
      notify("Failed to delete port", "error");
    }
  };
