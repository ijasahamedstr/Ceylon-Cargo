...
// ── Reusable ItemForm component (must be declared outside ManageAirManifest) ──
...
function ItemForm({ form, setForm }: ItemFormProps) {
  return (
...
function ManageAirManifest({ manifest, onBack }: { manifest: AirManifest; onBack: () => void }) {
  ...
  const fetchItems = useCallback(async () => {
    const r = await apiFetch(`/api/air-manifests/${manifest._id}`);
  ...
  const handleAdd = async () => {
    setSaving(true);
    const r = await apiFetch(`/api/air-manifests/${manifest._id}/items`, { method: "POST", body: JSON.stringify(form) });
  ...
  const handleEdit = async () => {
    if (!editItem) return;
    setSaving(true);
    const r = await apiFetch(`/api/air-manifest-items/${editItem._id}`, { method: "PUT", body: JSON.stringify(form) });
  ...
  const handleDelete = async () => {
    ...
    await apiFetch(`/api/air-manifest-items/${deleteId}`, { method: "DELETE" });
  ...
  return (
    <Box>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Button variant="contained" startIcon={<FileDownloadOutlined />} onClick={() => window.open(`${API_BASE_URL}/api/air-manifests/${manifest._id}/export`, "_blank")}
      ...
      <Dialog open={addOpen} onClose={() => { setAddOpen(false); setForm(empty); }} maxWidth="sm" fullWidth slotProps={{ paper: { sx: { borderRadius: "20px", p: 1 } } }}>
        ...
        <Box sx={{ px: 3, pb: 1 }}><ItemForm form={form} setForm={setForm} /></Box>
...
export default function AirManifestManager() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const r = await apiFetch("/api/air-manifests");
      const j = await r.json();
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    const url    = view === "edit" && selected ? `/api/air-manifests/${selected._id}` : `/api/air-manifests`;
  ...
  const handleDelete = async () => {
    ...
    await apiFetch(`/api/air-manifests/${id}`, { method: "DELETE" });
              ...
              {checked.length > 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <Button size="small" variant="contained" color="error" startIcon={<DeleteOutline />}
                    onClick={async () => { const updated = data.filter(d => !checked.includes(d._id)); setData(updated); localStorage.setItem(CACHE, JSON.stringify(updated)); const ids = [...checked]; setChecked([]); ids.forEach(async id => { await apiFetch(`/api/air-manifests/${id}`, { method: "DELETE" }); }); fetchData(); }}
